				...
				while(_g1 < uLen) {
					var u = _g1++;
					{
						...
						while(_g2 < vLen) {
							var v = _g2++;
							if(this.grid[u][v] < 4) {
								haxe.Log.trace("NOn floor detected over room!  " + i,{ fileName : "RoomFiller.hx", lineNumber : 220, className : "glidias.RoomFiller", methodName : "getSectors"});
								invalid = true;
								break;
							}
						}
	...
	var addedPortals = [];
	{
		...
		while(_g < len) {
			...
			if(doorType >= 4) {
				...
				haxe.Log.trace("indoors!" + [door.x,door.y] + " : " + [door.z,door.w],{ fileName : "RoomFiller.hx", lineNumber : 249, className : "glidias.RoomFiller", methodName : "getSectors"});
			}
			else if(doorType == 0) {
				target = -1;
				if(!this.enableOutdoors) continue;
				haxe.Log.trace("Outdoors!",{ fileName : "RoomFiller.hx", lineNumber : 254, className : "glidias.RoomFiller", methodName : "getSectors"});
			}
			else if(doorType == 1) {
				...
				haxe.Log.trace("Could not resolve door type. " + doorType + ". " + [door.x,door.y] + ": " + [door.z,door.w],{ fileName : "RoomFiller.hx", lineNumber : 318, className : "glidias.RoomFiller", methodName : "getSectors"});
				continue;
			}
			sector = new glidias.AABBSector();
			...
			if(target < 0) {
				haxe.Log.trace("Dead end.",{ fileName : "RoomFiller.hx", lineNumber : 368, className : "glidias.RoomFiller", methodName : "getSectors"});
				continue;
			}
