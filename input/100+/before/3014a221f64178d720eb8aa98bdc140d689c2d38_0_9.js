function() {
  var Bullet, DBEARING_MAX, HEIGHT, MAX_SENSOR_NOISE, Renderer, Tank, WIDTH, root;
  ...
  root.Game = (function() {
    ...
    Game.prototype.integrate = function(tanks) {
      ...
      for (_j = 0, _len1 = tanks.length; _j < _len1; _j++) {
        ...
        tank.fx -= tank.vx * 0.4;
        tank.fy -= tank.vy * 0.4;
        dx = Math.min(Math.max(tank.vx * this.dt + 0.5 * tank.fx * this.dt2, -50), 50);
        dy = Math.min(Math.max(tank.vy * this.dt + 0.5 * tank.fy * this.dt2, -50), 50);
        tank.vx = dx / this.dt;
        tank.vy = dy / this.dt;
        tank.x += dx;
        tank.y += dy;
