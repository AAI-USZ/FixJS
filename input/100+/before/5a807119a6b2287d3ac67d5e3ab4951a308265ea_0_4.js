function () {
  ...
  /* Note: get(), which calls isChanged(), which calls maybeChange(), performs
   ...
   */
  var maybeChange = function maybeChange(vv) {
    LOG("maybeChange: " + vv);
  ...
  var get = function get(vv) {
    ...
    DEBUG_BEGIN;
    if (vv.isChanged()) {
      LOG(vv + " is one side of a diamond dependency");
    }
    DEBUG_END;
  ...
  var set = function set(vv) {
    ...
    if (!vv.isChanged()) {
      ...
      if (vv.writtenBy) {
         ...
         * its writer. */
        LOG("ignoring identity assignment on " + vv);
      } else {
        /* If this variable is a source, then set() will be called with the
         * same value only by maybeChange. */
        LOG(vv + " is an unedited source variable");
      }
      DEBUG_END;
      return;
    }
     ...
     * are executed. In a diamond dependency, clearing usedBy here will forget
    ...
    = function update(touchedSet, newMethods, executedSetOut, changedSetOut)
  {
    ...
    newMethods.forEach(function (mm) {
      ...
      mm.needsExecution = true;
      if (mm.constraint) {
        var mmSelectedPrev = mm.constraint.selectedMethodPrev;
        if (mmSelectedPrev) {
          ASSERT(mmSelectedPrev !== mm,
            "expected new method to be different from old method");
          mmSelectedPrev.inputsUsed.forEach(function (vv) {
            vv.usedBy.remove(mmSelectedPrev)
          });
        }
      }
    });
