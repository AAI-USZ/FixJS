function () {
  ...
  template.trim = function trim() {
    return this.outgoing(validators.trim());
  }
  ...
  template.max = function max(maxval, errval) {
    return this.outgoing(validators.max(maxval, errval));
  }
  ...
  proxies.makeScratchProxy = function makeScratchProxy(vv) {
