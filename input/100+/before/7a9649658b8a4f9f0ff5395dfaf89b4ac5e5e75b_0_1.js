function (t, p) {
			...
			addData: function (data) { //parse data
				...
				if (p.dataType == 'json') {
					...
					$.each(data.rows, function (i, row) {
						...
						$('thead tr:first th[isch=true]', g.hDiv).each(function () {
						...
						$('thead tr:first th[isch=false]', g.hDiv).each( //add cell
							function () {
								var td = document.createElement('td');
					...
					$("rows row", data).each(function () {
						...
						$('thead tr:first th[isch=false]', g.hDiv).each(function () {
							var td = document.createElement('td');
							var idx = $(this).attr('axis').substr(3);
							td.align = this.align;
							td.innerHTML = p.showcheckbox?$("cell:eq(" + --idx + ")", robj).text():$("cell:eq(" + idx + ")", robj).text();
							$(td).attr('abbr', $(this).attr('abbr'));
							$(tr).append(td);
							td = null;
						});
			...
			addRowProp: function () {
				$('tbody tr', g.bDiv).each(function () {
					$(this).click(function (e) {
						...
						//add by Chrussy
						var ctd=$(this).find(':checkbox');
						if(ctd.attr('checked')){
						   ctd.attr('checked',false);
						}
						else
						{
						    ctd.attr('checked',true);
						}
						//end
		...
		$('thead tr:first th[isch=true]', g.hDiv).each(function (){
		   $(this).click(function(){
		        if(p.showcheckbox)
		        {
		             var checkth=$(this).find(':checkbox');
					 if(checkth.attr('checked')){
					    $('tbody tr',g.bDiv).each(function () {
