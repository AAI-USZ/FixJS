function (Module, Value, util, console, fs) {
	/**
	...
	function generateDetails(file){
		...
		for (var k in parsedModules) {
			if (parsedModules.hasOwnProperty(k)) {
				...
				fs.writeSync(fd, parseObject(module.value, module.id).toString(), null);
				...
				for(var c in properties){
					if (_hasOwnProperty.call(properties, c)) {
						...
						if (property.type === "constructor") {
							fs.writeSync(fd, parseObject(property, k + "." + c).toString(), null);
		...
		moduleArray.forEach(function(k) {
			...
			parts.forEach(function(part, idx){
				if(idx+1 < parts.length){
					...
					if(!lastChild || lastChild.id != id || lastChild.type != "folder"){
