function(p) {
      var index, order, period, timeFormat, timePeriod, _base, _base1, _base2, _base3, _base4;
      this.series = {};
      ...
      while (p.moreElements()) {
        ...
        if (p.moreComponents()) {
          if ((_base = this.series.obs).obsValue == null) {
            _base.obsValue = [];
          }
          this.series.obs.obsValue[index] = +p.get();
        }
        if (p.moreComponents()) {
          if ((_base1 = this.series.obs).attributes == null) {
            _base1.attributes = {};
          }
          if ((_base2 = this.series.obs.attributes)['OBS_STATUS'] == null) {
            _base2['OBS_STATUS'] = [];
          }
          this.series.obs.attributes['OBS_STATUS'][index] = p.get();
        }
        if (p.moreComponents()) {
          if ((_base3 = this.series.obs.attributes)['OBS_CONF'] == null) {
            _base3['OBS_CONF'] = [];
          }
          this.series.obs.attributes['OBS_CONF'][index] = p.get();
        }
        if (p.moreComponents()) {
          if ((_base4 = this.series.obs.attributes)['OBS_PRE_BREAK'] == null) {
