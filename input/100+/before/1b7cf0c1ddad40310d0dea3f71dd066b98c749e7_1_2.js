function addElementTabs(cleanContainerId, tabContainer, tabList, elementInfo, xml, parentPath, count, indent, existingElement) {
	...
	if (hasText) {
		...
		var valueValue = '';
		if(existingElement) {
			var nonSelectorElementTitle = elementInfo.elementTitle.replace(/\\/g,"");
	...
	if(hasElements) {
		...
		for (var i = 0; i < elementsArray.length; i++) {
			...
			addElementButton(elementInfo, cleanContainerId, addElementsContainer, xml, parentPath, elementsArray[i], count, indent);
		}
		
		// Add in existing children
		// order child nodes as they are in XML
		var se = xml.evaluate(parentPath+'/'+elementInfo.elementTitle.replace(/\\/g,"")+'['+count+']', xml, nsResolver, XPathResult.FIRST_ORDERED_NODE_TYPE, null);
		...
		for (var j = 0; j < se.singleNodeValue.childNodes.length; j++) {
			...
			for (var i = 0; i < elementsArray.length; i++) {
				if(elementsArray[i].elementTitle.replace(/\\/g,"") == nodeName) {
					elementHash[elementsArray[i].elementTitle] = elementHash[elementsArray[i].elementTitle] + 1;
					createElement(elementsArray[i], xml, parentPath+'/'+elementInfo.elementTitle.replace(/\\/g,"")+'['+count+']', elementHash[elementsArray[i].elementTitle], tabContent, 4);
				}
			}
		}
	}
	...
	if(hasAttributes) {
		...
		var attrPath = parentPath+'/'+elementInfo.elementTitle.replace(/\\/g,"");
