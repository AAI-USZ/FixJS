function (message) {
        switch ( message.command ) {
            ...
            case "NOTICE":
                ...
                break;
            case "MODE":
                log.debug("MODE:" + message.args[0] + " sets mode: " + message.args[1]);
            ...
            case "NICK":
                ...
                for ( var channame in self.chans ) {
                    ...
                    if ( 'string' == typeof channel.users[message.nick] ) {
                        channel.users[newNick] = channel.users[message.nick];
                ...
                if (message.nick === self.nick) {
                    self.nick = newNick;
                    self.emit('selfNick', message.nick, newNick, channels, message);
                } else {
                    // old nick, new nick, channels
                    self.emit('nick', message.nick, newNick, channels, message);
                }
            ...
            case "rpl_topic":
                ...
                if ( channel ) {
                    ...
                    self.emit('topic', message.args[1], channel.topic, null, null);
                }
                break;
            case "rpl_notopic":
                var channel = self.chanData(message.args[1]);
            ...
            case "JOIN":
                ...
                if ( self.nick == message.nick ) {
                    ...
                    var channel = self.chanData(message.args[0]);
                    if (!channel) { throw 'channel not found: ' + message.args[0]; }
