function () {
  ...
  var self = this;
  this.store.subscribe('dispatched', function(id, packets) {
    if (self.closed[id] && self.closed[id].length) {
      packets.forEach(function(packet) {
        var index = self.closed[id].indexOf(packet);
        if (index >= 0) {
          self.closed[id].splice(index, 1);
        }
      });
    }
  });
